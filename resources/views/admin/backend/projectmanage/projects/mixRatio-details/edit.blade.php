    ...
    <div class="content pb-0">
        ...
        <div class="row justify-content-center">
            ...
            <div class="card border-0 rounded-0">
                ...
                <div class="card-body">
                    <form action="{{ route('projectmanage.projects.mixRatio-details.update', [$project->id, $mixRatioDetail->id]) }}" method="POST"
                                    ...
                                    <select name="mix_ratio_template_id" id="mix_ratio_template_id" class="form-control form-select">
                                        ...
                                        @foreach ($mixRatios as $mixRatio)
                                            <option value="{{ $mixRatio->id }}"
                                                {{ old('mix_ratio_template_id', $mixRatioDetail->mix_ratio_template_id) == $mixRatio->id ? 'selected' : '' }}>
                                                {{-- {{ $mixRatioDetail->mix_ratio_template_id == $mixRatio->id ? 'selected' : '' }}> --}}
                                                {{ $mixRatio->code }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select name="variable_asset_id" id="variable_asset_id" class="form-control form-select">
                                        ...
                                        @foreach ($varilableAssets as $varilableAsset)
                                            <option value="{{ $varilableAsset->id }}"
                                                {{ $mixRatioDetail->variable_asset_id == $varilableAsset->id ? 'selected' : '' }}>{{ $varilableAsset->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
...
@endsection
@push('script')
     {!! JsValidator::formRequest('App\Http\Requests\MixRatioDetails\MixRatioDetailUpdateRequest', '#submit-form') !!}
@endpush
