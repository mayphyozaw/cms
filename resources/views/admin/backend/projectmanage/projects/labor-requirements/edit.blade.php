                    ...
                    <div class="card-body pb-0 pt-0 px-2">
                        ...
                        <ul class="nav nav-tabs nav-bordered nav-bordered-primary">
                            ...
                            <li class="nav-item me-3">
                                <a href="{{ route('projectmanage.projects.labor-requirements.index', $project->id) }}"
                                    class="nav-link p-2 {{ request()->routeIs('projectmanage.projects.labor-requirements.*') ? 'active' : '' }}">
                                    <i class="ti ti-settings-cog me-2"></i>
                                    Labor Requirements
                                </a>
                            </li>
                ...
                <div class="col-xl-12">
                    <div class="card">
                        <div class="card-header">
                            <h5 class="card-title">Labor Requirements Information</h5>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('projectmanage.projects.labor-requirements.update', [$project->id,$laborRequirement->id]) }}"
                                        ...
                                        <select name="drawing_measurement_id" id="drawing_measurement_id"
                                            ...
                                            @foreach ($drawingMeasurements as $drawingMeasurement)
                                                <option value="{{ $drawingMeasurement->id }}"
                                                    {{ $laborRequirement->drawing_measurement_id == $drawingMeasurement->id ? 'selected' : '' }}>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <div class="input-group">
                                            <input type="hidden" name="labor_type_id" class="form-control"
                                                id="labor_type_id">
                                            <select name="labor_mapping_id" id="labor_mapping_id"
                                                class="form-control select2">
                                                <option value="">Select Labor Mapping</option>
                                                @foreach ($laborMappings as $laborMapping)
                                                    <option value="{{ $laborMapping->id }}"
                                                        {{ $laborRequirement->labor_mapping_id == $laborMapping->id ? 'selected' : '' }}>
                                                        {{ $laborMapping->laborType->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                    ...
                                    <div class="col-sm-9">
                                        <div class="input-group">
                                            <input type="text" name="raw_quantity" class="form-control"
                                                id="quantity" value="{{ $laborRequirement->raw_quantity }}" readonly>
                                            <div class="input-group-text">
                                                <input type="text" name="unit" class="form-control" id="unit" value="{{ $laborRequirement->drawingMeasurement->unit }}"
                                                    readonly>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-sm-9">
                                        <div class="input-group">
                                            <input type="text" name="productivity" class="form-control"
                                                id="productivity" value="{{ $laborRequirement->laborMapping->productivity }}" readonly>
                                            <div class="input-group-text">
                                                <input type="text" class="form-control" id="labor_unit" value="{{ $laborRequirement->laborMapping->unit }}" readonly>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-sm-9">
                                        <div class="input-group">
                                            <input type="text" name="required_qty" class="form-control required_qty"
                                                id="required_qty" value="{{ $laborRequirement->required_qty }}" readonly>
                                            <div class="input-group-text">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-9">
                                        <div class="input-group">
                                            <input name="status" class="form-control"  value={{ $laborRequirement->status }}/>
                                        </div>
                                    </div>
                                </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            $('#labor_mapping_id').on('change', function() {

                let laborMappingId = $(this).val();

                $.ajax({
                    url: "{{ route('projectmanage.labor_mapping_get') }}",
                    type: 'GET',
                    data: {
                        labor_mapping_id: laborMappingId
                    },
                    success: function(data) {

                        $('#labor_type_id').val(data.labor_type_id);

                        $('#productivity').val(data.productivity);

                        $('#labor_unit').val(data.unit);
            ...
            function calculateRequireQuantity() {
                ...
                let requiredQty = productivity > 0 ?
